(function(){
  ...
  function html(d){
    ...
    return `<section class="lm-ticker-search-radar" id="lmTickerSearchRadar" style="--lm-tsr-speed:${speed}">
      <div class="lm-tsr-head"><div><div class="lm-eyebrow">Search Stock Options Radar</div><h2>Ticker Laser Reader</h2><p>Search any ticker to generate a stock view with a buy/sell gauge, a larger 1-minute chart, and a written pressure description.</p></div><form class="lm-tsr-form" data-tsr-form><input data-tsr-input placeholder="Ticker e.g. AAPL" value="${esc(d.symbol)}"><button type="submit">Generate Stock View</button></form></div>
      <div class="lm-tsr-grid">
        ...
        <article class="lm-tsr-read"><h3>${esc(d.symbol)} Quick Read</h3><p>${d.bias==='Bullish'?'Bullish pressure is winning right now in this reader.':'Bearish pressure is winning right now in this reader.'}</p><div class="lm-tsr-stat"><span>Pressure</span><strong>${d.pressure}%</strong></div><div class="lm-tsr-stat"><span>Volume base</span><strong>${d.volume}</strong></div><div class="lm-tsr-stat"><span>Signal type</span><strong>${d.bias==='Bullish'?'Bullish / call pressure':'Bearish / put pressure'}</strong></div></article>
