(function(){
  ...
  const RECENT_KEY = 'lmRecentAltcoinSearches';
  ...
  function remember(symbol){const clean=String(symbol||'').toUpperCase().replace(/[^A-Z0-9]/g,'').slice(0,10);if(!clean)return;const next=[clean,...readRecent().filter(x=>x!==clean)].slice(0,6);localStorage.setItem(RECENT_KEY,JSON.stringify(next));localStorage.setItem('lmLastAltcoin',clean)}
  function recentHtml(){const recent=readRecent();return recent.length?`<div class="lm-alt-recents"><span>Recent crypto</span>${recent.map(t=>`<button type="button" data-alt-recent="${esc(t)}">${esc(t)}</button>`).join('')}</div>`:''}
  ...
  function css(){
    ...
    s.textContent=`
      ...
      .lm-alt-head{display:flex;justify-content:space-between;align-items:flex-start;gap:1rem;flex-wrap:wrap}.lm-alt-head h2{margin:.15rem 0}.lm-alt-form{display:flex;gap:.5rem;flex-wrap:wrap}.lm-alt-form input{border:1px solid rgba(255,255,255,.14);border-radius:12px;background:rgba(0,0,0,.35);color:#fff;padding:.72rem;font-weight:900;text-transform:uppercase}.lm-alt-form input[type=number]{width:115px}.lm-alt-form button{border:0;border-radius:12px;background:#ffc542;color:#06101f;font-weight:1000;padding:.72rem 1rem;cursor:pointer}.lm-alt-recents{display:flex;align-items:center;gap:.4rem;flex-wrap:wrap;margin-top:.55rem}.lm-alt-recents span{color:rgba(255,255,255,.62);font-size:.72rem;font-weight:900;text-transform:uppercase}.lm-alt-recents button{border:1px solid rgba(255,255,255,.14);border-radius:999px;background:rgba(255,255,255,.055);color:#fff;padding:.32rem .55rem;font-size:.72rem;font-weight:900;cursor:pointer}
  ...
  function calc(coin,budget,slot=0){
    ...
    const laserSpeed=Number(Math.max(.28,2.18-(score/100)*1.42+slot*.13).toFixed(2));
    const speedLabel=laserSpeed<=.55?'flash':laserSpeed<=.95?'rapid':laserSpeed<=1.35?'quick':'steady';
    return {coin:s,pair:pair(s),score,bull,bear,demand,supply,bias,price,volatility,budget:Number(budget)||200,source:'24/7 altcoin analytics radar',candles:makeCandles(s,score),laserSpeed,speedLabel};
  }
  ...
  function readerCard(d){
    ...
    return `<article class="lm-alt-reader" style="--reader-speed:${d.laserSpeed}s"><div class="lm-alt-reader-top"><strong>${esc(d.coin)}</strong><span class="lm-alt-reader-score ${paid?'':'lm-alt-score-locked'}">${scoreText(d.score)}</span></div><canvas class="lm-alt-mini" data-alt-mini="${esc(d.coin)}"></canvas><div class="lm-alt-speed-read"><span>Laser speed</span><strong>${esc(d.speedLabel)} ${d.laserSpeed}s</strong><span class="lm-alt-speed-track"></span></div><div class="lm-alt-stat"><span>Pair</span><strong>${esc(d.pair)}</strong></div>${gauge(d)}${paid?'':`<a class="lm-alt-buy-link" href="${MARKET_URL}" target="_blank" rel="noopener">Unlock actual score</a>`}</article>`;
  }
  ...
  function html(d, readers){
    ...
    return `<section class="lm-altcoin-laser" id="lmAltcoinLaser" style="--lm-alt-speed:${speed}">
      <div class="lm-alt-head"><div><div class="lm-eyebrow">Search Altcoin Radar</div><h2>4 Altcoin Readers + Buy/Sell Gauge</h2><p>Free accounts remember recent crypto searches. $25,000 Market Signals unlocks the actual crypto and stock reading scores.</p>${paid?'':`<a class="lm-alt-buy-link" href="${MARKET_URL}" target="_blank" rel="noopener">Buy LasersMarket Access</a>`}${recentHtml()}</div><form class="lm-alt-form" data-alt-form><input data-alt-coin placeholder="SOL" value="${esc(d.coin)}"><input data-alt-budget type="number" min="1" step="1" value="${esc(d.budget)}"><button type="submit">Generate Altcoin View</button></form></div>
      <div class="lm-alt-reader-grid">${readers.map(readerCard).join('')}</div>
      <div class="lm-alt-grid"><article class="lm-alt-card"><div class="lm-alt-symbol">${esc(d.coin)}</div><div class="lm-alt-pair">${esc(d.pair)} - $${esc(d.price)}</div><div class="lm-alt-lasers"><span></span><span></span><span></span><span></span></div><div class="lm-alt-stat"><span>Laser Score</span><strong>${scoreText(d.score)}</strong></div><div class="lm-alt-stat"><span>Laser Speed</span><strong>${esc(d.speedLabel)} ${d.laserSpeed}s</strong></div><div class="lm-alt-stat"><span>Bias</span><strong class="${d.bull>=d.bear?'lm-alt-bull':'lm-alt-bear'}">${esc(d.bias)}</strong></div><div class="lm-alt-stat"><span>Volatility Read</span><strong>${d.volatility}%</strong></div></article>
  ...
  document.addEventListener('submit',e=>{const form=e.target.closest('[data-alt-form]');if(!form)return;e.preventDefault();const coin=form.querySelector('[data-alt-coin]')?.value||DEFAULT;remember(coin);render(coin,form.querySelector('[data-alt-budget]')?.value||200)},true);
  ...
  document.addEventListener('click',e=>{const recent=e.target.closest('[data-alt-recent]');if(recent){remember(recent.dataset.altRecent);render(recent.dataset.altRecent,STATE.budget)}},true);
  document.addEventListener('lm-auth-updated',()=>render(STATE.coin,STATE.budget),true);
  function start(){render(localStorage.getItem('lmLastAltcoin')||DEFAULT,200);clearInterval(STATE.timer);STATE.timer=setInterval(()=>{STATE.rotation=(STATE.rotation+4)%WATCH.length;render(STATE.coin,STATE.budget)},45000)}
