(function(){
  ...
  const rows = [
    {slot:'#1',symbol:'RBLX',market:'SPY watch universe',date:'2026-04-29 overnight to 2026-04-30',price:'54.45',premium:'$25,000 tier',cp:'n/a / n/a',bias:'bearish building',route:'10m scanner',score:87},
    {slot:'#2',symbol:'SNAP',market:'SPY watch universe',date:'2026-04-29 overnight to 2026-04-30',price:'5.63',premium:'$25,000 tier',cp:'n/a / n/a',bias:'bearish building',route:'10m scanner',score:62},
    {slot:'#3',symbol:'FUBO',market:'SPY watch universe',date:'2026-04-29 overnight to 2026-04-30',price:'12.34',premium:'$25,000 tier',cp:'n/a / n/a',bias:'bearish scanning',route:'10m scanner',score:35},
    {slot:'#4',symbol:'PINS',market:'SPY watch universe',date:'2026-04-29 overnight to 2026-04-30',price:'19.73',premium:'$25,000 tier',cp:'n/a / n/a',bias:'bearish unhooked',route:'10m scanner',score:28}
  ];
  const history = [
  ...
  function readAuth(){try{return JSON.parse(localStorage.getItem('lmAuthV1')||'{}')}catch{return {}}}
  ...
  function isRegistered(){const l=read();const a=readAuth();return (!!l.verified && !!(l.email || l.phone)) || (!!a.verified && !!(a.email || a.phone))}
  function hasPaidAccess(){const l=read();const a=readAuth();const plan=String(a.plan||a.tier||l.tier||'');return owner(l.email||a.email,l.phone||a.phone) || l.paid === true || /signals|25000/i.test(plan) || localStorage.getItem('lmMarketSignalsPaid') === 'true' || localStorage.getItem('lasersMarketPaidAccess') === 'true'}
  function accessLabel(){if(!isRegistered())return 'Register to continue';return hasPaidAccess()?'$25,000 Market Signals unlocked':'Registered - $25,000 checkout required'}
  function saveLogin(email,ph,provider){
    ...
    return write({
      ...
      verified:true,
      paid:ok,
      tier:ok?'Verified Owner Access':'Verified Member - $25,000 Checkout Required',
      username:ok?'DYGENRJE':(String(email||'').split('@')[0]||'Member'),
      at:new Date().toISOString()
    });
  }
  function markPaid(){const l=read();write({...l,verified:true,paid:true,tier:'$25,000 Market Signals Access',paidAt:new Date().toISOString()});localStorage.setItem('lasersMarketPaidAccess','true');localStorage.setItem('lmMarketSignalsPaid','true')}
  ...
  function loginHtml(){
    const l=read();
    if(isRegistered()){
      return `<h3>Verified Login</h3><p class="lm-note"><strong>${esc(l.email||'Email not saved')}</strong><br>${esc(l.phone||'Phone not saved')}<br>${esc(l.tier||'Verified Member')}</p>${hasPaidAccess()?'<div class="lm-verified-banner">$25,000 Market Signals access is active.</div>':`<div class="lm-pay-card"><strong>$25,000 checkout required</strong><p class="lm-note">Register is complete. Pay for LasersMarket Signals to unlock the full tracker and actual reading scores.</p><a href="${CHECKOUT_URL}" target="_blank" rel="noopener">Go to Checkout</a><button class="ghost" data-lm-mark-paid>Mark Paid After Checkout</button></div>`}<button class="ghost" data-lm-edit>Switch Account</button>`;
    }
    return `<h3>LasersMarket Login</h3><p class="lm-note">Register free first. The only paid tier is $25,000 Market Signals.</p><input data-lm-email placeholder="Email"><input data-lm-phone placeholder="Phone number"><div><button data-lm-provider="gmail">Gmail</button><button data-lm-provider="microsoft">Microsoft</button><button data-lm-provider="apple">Apple</button></div><button data-lm-save>Register / Login</button><button class="ghost" data-lm-returning-vip>Returning $25,000 Login</button><p class="lm-note">Free account opens the page. $25,000 access unlocks actual crypto and stock reading scores.</p>`
  }
  function renderLogin(){
    document.querySelector('#lmLoginBox')?.remove();
  }
  function tableHtml(data){return `<table class="lm-table"><thead><tr>${['Slot','Symbol','Market','Date','Price','$25,000 Tier','Calls/Puts','Bias','Route','Score'].map(x=>`<th>${x}</th>`).join('')}</tr></thead><tbody>${data.map(r=>`<tr><td>${r.slot}</td><td><strong>${r.symbol}</strong></td><td>${r.market}</td><td>${r.date}</td><td>${r.price}</td><td>${r.premium}</td><td>${r.cp}</td><td>${r.bias}</td><td>${r.route}</td><td>${r.score}</td></tr>`).join('')}</tbody></table>`}
  ...
  function renderPanel(){
    ...
    mount.innerHTML=`<div class="lm-global-head"><div><div class="lm-eyebrow">LasersMarket $25,000 Market Signals</div><p>Market rows are analytics and research only. They do not execute trades, guarantee performance, or recommend a buy/sell decision. $25,000 access unlocks the full signal view and actual reading scores.</p></div><div class="lm-lock">${accessLabel()}</div></div>${!isRegistered()?'<div class="lm-pay-card"><strong>Register first</strong><p>Enter email and phone in the login panel to create a free account.</p></div>':(!unlocked?`<div class="lm-pay-card"><strong>$25,000 payment required</strong><p>Registration is complete. Checkout unlocks the whole LasersMarket Signals view.</p><a href="${CHECKOUT_URL}" target="_blank" rel="noopener">Checkout for $25,000 Access</a><button data-lm-mark-paid>Mark Paid After Checkout</button></div>`:'<div class="lm-verified-banner">$25,000 Market Signals unlocked for this verified account.</div>')}<h2>Live tracked feed - 4 to 5 at a time</h2><div class="${unlocked?'':'lm-locked-blur'}">${tableHtml(rows)}<h2>Historical values - separate 10 day competition</h2>${historyHtml()}</div><div class="lm-actions"><button data-lm-refresh>Refresh Reading</button>${unlocked?'<button class="ghost" data-lm-edit>Switch Account</button>':`<a href="${CHECKOUT_URL}" target="_blank" rel="noopener">Go to Checkout</a>`}</div>`;
  ...
  document.addEventListener('click',e=>{const provider=e.target.closest('[data-lm-provider]');if(provider){const box=document.querySelector('#lmLoginBox');if(box)box.dataset.provider=provider.dataset.lmProvider;alert(provider.dataset.lmProvider+' selected. Connect OAuth credentials server-side for live provider login.')}if(e.target.closest('[data-lm-save]')){const box=document.querySelector('#lmLoginBox');const email=box.querySelector('[data-lm-email]')?.value||'';const ph=box.querySelector('[data-lm-phone]')?.value||'';saveLogin(email,ph,box.dataset.provider||'email');rerender()}if(e.target.closest('[data-lm-returning-vip]')){const box=document.querySelector('#lmLoginBox');const email=box.querySelector('[data-lm-email]')?.value||'';const ph=box.querySelector('[data-lm-phone]')?.value||'';write({email:String(email||'').trim(),phone:phone(ph),provider:box.dataset.provider||'email',verified:true,paid:true,tier:'$25,000 Market Signals Access',username:(String(email||'').split('@')[0]||'Member'),returning:true,paidAt:new Date().toISOString()});localStorage.setItem('lmMarketSignalsPaid','true');localStorage.setItem('lasersMarketPaidAccess','true');rerender()}if(e.target.closest('[data-lm-mark-paid]')){markPaid();rerender()}if(e.target.closest('[data-lm-edit]')){localStorage.removeItem(LOGIN_KEY);localStorage.removeItem('lasersMarketPaidAccess');rerender()}if(e.target.closest('[data-lm-refresh]'))renderPanel();});
